...
var sizeStyle = function sizeStyle(props) {
  ...
  var width = props.fillContainer
    ? '100%'
    : props.theme.global.size[props.sizeProp];
  return (0, _styledComponents.css)(
    ['font-size:', ';line-height:', ';width:', ';', ''],
    data.fontSize,
    data.lineHeight,
    width,
    function(p) {
...
var weeksContainerSizeStyle = function weeksContainerSizeStyle(props) {
  var height = props.fillContainer
    ? '100%'
    : (0, _utils.parseMetricToNum)(
        props.theme.calendar[props.sizeProp].daySize,
      ) *
        6 +
      'px';
  return '\n    height: ' + height + ';\n\n  ';
...
var slideStyle = function slideStyle(props) {
  ...
  var slideTransition = (0, _styledComponents.css)(
    ['0%{transform:translateY(', ');}100%{transform:translateY(', ');}'],
    translateYFrom,
    translateYTo,
  );
  return (0, _styledComponents.css)(
    ['animation:', ' ', ' forwards;'],
    (0, _styledComponents.keyframes)(['', ''], slideTransition),
    slideDuration,
  );
};
...
var daySizeStyle = function daySizeStyle(props) {
  ...
  return (0, _styledComponents.css)(
    ['width:', ';height:', ';'],
    props.fillContainer ? '100%' : data.daySize,
    props.fillContainer ? '100%' : data.daySize,
  );
};
...
var StyledDay = _styledComponents['default'].div.withConfig({
  ...
  function(props) {
    return (
      (props.isSelected &&
        (0, _utils.backgroundStyle)('control', props.theme)) ||
      (props.inRange &&
  ...
  function(props) {
    return (
      ...
      (0, _utils.kindPartStyles)(props.theme.button.disabled, props.theme)
    );
  },
  function(props) {
    return (
