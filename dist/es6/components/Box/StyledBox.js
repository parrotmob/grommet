...
var elevationStyle = css(['box-shadow:', ';'], function(props) {
  return props
    .theme.global.elevation[props.theme.dark ? 'dark' : 'light'][props.elevationProp];
});
var FLEX_MAP =
  ((_FLEX_MAP = {}),
  (_FLEX_MAP[true] = '1 1'),
...
var animationStyle = css(['', ';'], function(props) {
  return css(
    ...
    animationItemStyle(props.animation, props.theme),
  );
});
...
var heightObjectStyle = css(
  ['', ';', ';'],
  function(props) {
    return (
      props.heightProp.max &&
      css(['max-height:', ';'], getSize(props, props.heightProp.max))
    );
  },
  function(props) {
    return (
      props.heightProp.min &&
      css(['min-height:', ';'], getSize(props, props.heightProp.min))
    );
  },
);
...
var widthObjectStyle = css(
  ['', ';', ';', ';'],
  function(props) {
    return (
      props.widthProp.max &&
      css(['max-width:', ';'], getSize(props, props.widthProp.max))
    );
  },
  function(props) {
    return (
      props.widthProp.min &&
      css(['min-width:', ';'], getSize(props, props.widthProp.min))
    );
  },
  function(props) {
    return (
      props.widthProp.width &&
      css(['width:', ';'], getSize(props, props.widthProp.width))
    );
  },
);
...
var gapStyle = function gapStyle(
  ...
  if (directionProp === 'column' || directionProp === 'column-reverse') {
    ...
    if (responsiveMetric) {
      styles.push(
        breakpointStyle(breakpoint, 'height: ' + responsiveMetric + ';'),
      );
    }
  } else {
    ...
    if (responsiveMetric) {
      if (directionProp === 'row' || directionProp === 'row-reverse') {
        styles.push(
          breakpointStyle(breakpoint, 'width: ' + responsiveMetric + ';'),
        );
  ...
  if (border === 'between' || (border && border.side === 'between')) {
    ...
    var borderOffset =
      parseMetricToNum(metric) / 2 - parseMetricToNum(borderMetric) / 2 + 'px';
