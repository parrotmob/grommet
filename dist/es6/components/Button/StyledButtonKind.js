...
var radiusStyle = function radiusStyle(props) {
  ...
  if (size && themeObj.size && themeObj.size[size])
    return css(['border-radius:', ';'], themeObj.size[size].border.radius);
  if (themeObj.border && themeObj.border.radius)
    return css(['border-radius:', ';'], themeObj.border.radius);
  return '';
};
...
var adjustPadStyle = function adjustPadStyle(pad, width) {
  ...
  return css(
    ['padding:', 'px ', 'px;'],
    Math.max(parseMetricToNum(pad.vertical) - offset, 0),
    Math.max(parseMetricToNum(pad.horizontal) - offset, 0),
  );
...
var StyledButtonKind = styled.button
  .withConfig({
